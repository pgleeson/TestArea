...
NEURON {
    ...
    RANGE rest_conc, tau, F, thickness, shell_vol, surf_area
...
INITIAL {
    ...
    shell_vol = (diam*diam*diam)*3.14159/6 - (shell_inner_diam*shell_inner_diam*shell_inner_diam)*3.14159/6
...
DERIVATIVE conc {
    LOCAL thickness_cm, surf_area_cm2, shell_vol_cm3 ? Note, normally dimensions are in um, but curr dens is in mA/cm2, etc
    
    thickness_cm = thickness *(1e-4)
    surf_area_cm2 = surf_area * 1e-8
    shell_vol_cm3 = shell_vol * 1e-12
    
    total_current = i<xsl:value-of select="$ionused"/> * surf_area_cm2



    <xsl:value-of select="$ionused"/>i' =  ((-1 * total_current)/(<xsl:value-of select="$valency"/> * F * shell_vol_cm3)) - ((<xsl:value-of select="$ionused"/>i - rest_conc)/tau)
