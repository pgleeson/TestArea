#
#   A file that opens the neuroConstruct project LarkumEtAl2009 and run multiple simulations triggering NMDA spikes in random apical branches.
#
...
if numGenerated > 0:
    ...
    for y in range(0, Nbranches):
       ...
       prefix = "s05" #remember to change the glutamate file to match the required conductance
	  ...
	  stim = myProject.elecInputInfo.getStim("NMDAspike_focal")
	  location = stim.getSegChooser()
	  location.setGroup(apical_branch[randomApicalBranch])
          location.setNumberOfSegments(1)
	  ...
	  print "NMDAspike in "+apical_branch[randomApicalBranch]
          
          ##########################################################################################'''
	  
	  simRef = prefix+"_"+str(i)+"_ID"+str(apical_ID[randomApicalBranch])
	  print "Simref: "+simRef
