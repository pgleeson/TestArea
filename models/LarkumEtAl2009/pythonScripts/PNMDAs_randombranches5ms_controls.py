...
if numGenerated > 0:
    ...
    for s in range(1, syns/3):
	  ...
	  if compileSuccess:
	     pm.doRunNeuron(simConfig)
	     print "Set running simulation: "+simRef
	  time.sleep(10) # wait for the process to be sent out  
	  
	  simInputs.remove("apical_IClamp")	 
	  ...
	  simInputs.add("apical_IClamp_1200")	  
	  simConfig.setInputs(simInputs)
	  ...
	  simInputs.remove("apical_IClamp_1200")	 
	  ...
	  simInputs.add("apical_IClamp_900")	  
	  simConfig.setInputs(simInputs)
	  ...
	  simInputs.remove("apical_IClamp_900")
	  
	  #####################'''
	  
	  ##### Rerunning the same configuration + AMPA 1500 #####
	  
	  simRef = prefix+"syn"+str(synapses)+"AMPA1500_"+str(t)
	  print "Simref: "+simRef
	  myProject.simulationParameters.setReference(simRef)
	  refStored.append(simRef)
	  ...
	  if compileSuccess:
	     pm.doRunNeuron(simConfig)
	     print "Set running simulation: "+simRef
	  time.sleep(1) # wait for the process to be sent out 
	  
	  simInputs.remove("AMPAbackground")
	  ...
	  simInputs.add("AMPAbackground_1200")	  
	  simConfig.setInputs(simInputs)
	  ...
	  simInputs.remove("AMPAbackground_1200")
	  ...
	  simInputs.add("AMPAbackground_900")	  
	  simConfig.setInputs(simInputs)
	  ...
	  simInputs.remove("AMPAbackground_900")
