...
if numGenerated > 0:
    ...
    ms = 50
    Hz = 0.02
    syns = 260 #will run in steps of 10 synpases
    
    prefix = "noCDId50ms" #string that will be added to the name of the simulations to identify the simulation set    
    ...
    for s in range(1, syns/10):
	  ...
	  #  adjusting synchonous distributed input 
	  ...
	  print str(synapses)+" syncrhonous distributed synapses firing at "+str(Hz*ms)+" each"
	  ...
	  if (s<150): ##### Rerunning the same configuration + background exc #####
	    ...
	    simRef = prefix+"syn"+str(synapses)+"EI150_"+str(t)
	    print "Simref: "+simRef
	    myProject.simulationParameters.setReference(simRef)
	    refStored.append(simRef)
