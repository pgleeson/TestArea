...
extern int nrn_get_mechtype();
 ...
 "states_sca", _hoc_states,
 "trates_sca", _hoc_trates,
 0, 0
};
#define efun efun_sca
 extern double efun();
	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 3);
 	...
 	ivoc_help("help ?1 sca /Users/danielruedt/PhD/simulations/LarkumEtAl2009_template_ratecoding/umac/SlowCa.mod\n");
...
static int _hoc_states() {
 ...
 static _check_trates() {
  ...
  static double _sav_inactF;
  if (!usetable) {return;}
  if (_sav_dt != dt) { _maktable = 1;}
  if (_sav_celsius != celsius) { _maktable = 1;}
  if (_sav_temp != temp) { _maktable = 1;}
  if (_sav_inactF != inactF) { _maktable = 1;}
...
static int _ode_count(_type)int _type; { hoc_execerror("sca", "cannot be used with CVODE");}
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 75 in file SlowCa.mod:\n        SOLVE states\n"); nrn_complain(_p); abort_run(error);}
