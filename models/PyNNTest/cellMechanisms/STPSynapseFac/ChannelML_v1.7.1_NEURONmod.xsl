...
<xsl:if test="count(cml:stdp_syn)>0">
    ...
    RANGE weight_factor                : multiplicative factor for weight which changes based on activity
...
BREAKPOINT {
    ...
    if (in_post_spike == 0 &amp;&amp; v > post_spike_thresh) {
        ...
        weight_factor = weight_factor + deltaw
        <xsl:if test="$debug = 1">
        printf("-- weight_factor: %g, deltaw: %g, P: %g, M: %g\n", weight_factor,deltaw, P, M)
        </xsl:if>
