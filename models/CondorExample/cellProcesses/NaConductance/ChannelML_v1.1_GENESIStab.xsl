...
function make_<xsl:value-of select="@name"/>
        ...
        */
        ...
        echo "Temperature adjustment factor: " {temp_adj}
                </xsl:when>
                <xsl:otherwise>
        ...
        for (i = 0; i &lt;= ({tab_divs}); i = i + 1)
            ...
            //echo "tab <xsl:value-of select="$gateRef"/>: i: "{i}", v: "{v} ", alpha: "{alpha}", beta: "{beta}", tau: "{tau} 
            ...
            //echo "tab <xsl:value-of select="$gateRef"/>: v: "{v} ", tau: "{tau} ", inf: "{inf}
