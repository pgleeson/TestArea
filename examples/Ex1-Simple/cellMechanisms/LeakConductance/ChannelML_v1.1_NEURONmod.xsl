...
NEURON {
    ...
    USEION <xsl:value-of select="@name"/> READ <xsl:value-of select="@name"/>i ? internal concentration of ion is read
            </xsl:when>
            <xsl:when test ="@role='ConcVaries'">
    USEION <xsl:value-of select="@name"/> READ i<xsl:value-of select="@name"/> WRITE <xsl:value-of select="@name"/>i  ? outgoing current of ion is read, internal concentration is written
            </xsl:when>
            <xsl:otherwise>
    USEION <xsl:value-of select="@name"/> READ e<xsl:value-of select="@name"/> WRITE i<xsl:value-of select="@name"/> ? reversal potential of ion is read, outgoing current is written
            </xsl:otherwise>
        </xsl:choose>
...
BREAKPOINT {
    ...
    i = gmax*(v - e) 
        ...
        <xsl:otherwise>
    <xsl:choose><xsl:when test="$voltConcDependence='yes'">SOLVE states METHOD derivimplicit</xsl:when>
    <xsl:otherwise>SOLVE states METHOD cnexp</xsl:otherwise></xsl:choose>
...
NEURON {
    ...
    <xsl:for-each select="/cml:channelml/cml:ion[@name!='non_specific']">
    USEION <xsl:value-of select="@name"/> READ i<xsl:value-of select="@name"/> WRITE <xsl:value-of select="@name"/>i
...
DERIVATIVE conc {
    ...
    <xsl:value-of select="$ionused"/>i' = (-i<xsl:value-of select="$ionused"/>/(<xsl:value-of select="$valency"/> * F * thickness *(1e-4))) - ((<xsl:value-of select="$ionused"/>i - rest_conc)/tau)
